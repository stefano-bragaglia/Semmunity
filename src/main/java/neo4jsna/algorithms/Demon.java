...
import neo4jsna.utils.GraphUtils;
import org.neo4j.graphdb.*;

import static neo4jsna.algorithms.Demon.DemonRelType.DEMON_RELTYPE;

/**
 * Created by besil on 26/05/15.
 */
public class Demon implements SingleNodeScanAlgorithm {
	public static final Label DemonLabel = Label.label("DEMON_NODE");
	...
	public Demon(GraphDatabaseService db) {
		...
		this.lp = new LabelPropagation(DEMON_RELTYPE);
	}
		...
		for (long neigh : neighbours) {
			...
			for (Relationship r : neighNode.getRelationships(Direction.OUTGOING)) {
				...
				if (neighbours.contains(other.getId())) {
					neighNode.createRelationshipTo(other, DEMON_RELTYPE);
					relMarked++;
				}
			}
		}
	...
	public Iterator<Relationship> getDemonRelationships() {
		return GraphUtils.getRelationshisByNodeAndRelationshipType(db, Demon.DemonLabel, DEMON_RELTYPE);
	}

	/**
	 * Mark the nodes of the egoNetwork of node root with label DemonLabel and create the relationships of type
	 * DemonRelType
	 *
	 * @param root
	 * @return
	...
	public void setEgoNetwork(Node root) {
		...
		for (Relationship r : root.getRelationships(Direction.OUTGOING)) {
			...
			neigh.addLabel(DemonLabel);
			root.createRelationshipTo(neigh, DEMON_RELTYPE);
			neighbours.add(neigh.getId());
		}
	...
	public void clearEgoNetwork(Node root) {
		Iterator<Relationship> demonRels = GraphUtils
				.getRelationshisByNodeAndRelationshipType(db, DemonLabel, DEMON_RELTYPE);
