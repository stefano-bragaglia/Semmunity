...
import neo4jsna.measures.UndirectedModularity;
...
public class Louvain {
	...
	public Louvain(GraphDatabaseService g) {
		...
		this.g = g;
		this.layerLabel = Label.label("layerLabel");
		this.communityLabel = Label.label("communityLabel");
		this.newLayerLabel = Label.label("newLayerLabel");
...
//                logger.info("Src: " + src);
				...
				Iterable<Relationship> rels = layerCount == 0 ?
						src.getRelationships(Direction.BOTH) :
						src.getRelationships(Direction.BOTH, LouvainRels.NewEdges);
...
//                    logger.info("    Dst: " + neigh + " -> " + delta);
	...
	private double calculateDelta(Node n, long srcCommunity, long dstCommunity) {
		...
		second = (this.communityVolumeWithout(n, srcCommunity) - this
				.communityVolumeWithout(n, dstCommunity)) * nodeVolume(n);
	...
	public int secondPhase() {
		...
		if (macroNodesCommunities.size() == macroNodeCount) {
			...
			while (activeNodes.hasNext()) {
				...
				long activeNodeId = activeNode.hasProperty("id") ? (int) activeNode.getProperty("id") : activeNode
						.getId();
		...
		while (activeNodes.hasNext()) {
			...
			Node activeNode = activeNodes.next();
			long activeNodeId = activeNode.hasProperty("id") ? Long
					.parseLong((String) activeNode.getProperty("id")) : activeNode.getId();
			long cId = (long) activeNode.getProperty(communityProperty);
			...
			for (Relationship layer : macroNode.getRelationships(Direction.INCOMING, LouvainRels.Layer)) {
				...
				for (Relationship r : originalNode.getRelationships(Direction.BOTH)) {
					if (!r.isType(LouvainRels.Layer)) {
						...
						Node otherMacroNode = neigh.getSingleRelationship(LouvainRels.Layer, Direction.OUTGOING)
								.getOtherNode(neigh);

						Relationship macroRel = getRelationshipBetween(macroNode,
																	   otherMacroNode,
																	   Direction.BOTH,
																	   LouvainRels.NewEdges);
						if (macroRel == null) {
